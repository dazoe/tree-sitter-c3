...
export default grammar({
  ...
  inline: ($) => [$._statement, $._top_level_item],
  ...
  rules: {
    ...
    _doc_comment_description: ($) =>
      seq(optional(":"), field("description", $.string_expr)),
    ...
    doc_comment_contract: ($) =>
      choice(
        seq(
          ...
          optional(
            field("mutability_contract", $.doc_comment_contract_descriptor),
          ),
        ...
        seq(
          ...
          choice(
            seq("?", commaSep1($._expr), optional($._doc_comment_description)),
    ...
    _decl_ident: ($) => choice($.ident, $.ct_ident),

    _arg_ident: ($) =>
      choice($.ident, $.ct_ident, $.ct_type_ident, $.hash_ident),

    _func_macro_ident: ($) => choice($.ident, $.at_ident),

    _expr_ident: ($) => choice($.const_ident, $.ident, $.at_ident),

    _local_expr_ident: ($) => choice($.ct_ident, $.hash_ident),

    // Module Paths
    // -------------------------
    module_resolution: ($) => prec.left(seq($.ident, "::")),
    ...
    // Attributes
    // -------------------------
    _attribute_name: ($) => choice($.at_ident, $.path_at_type_ident),
    ...
    attribute: ($) =>
      seq(field("name", $._attribute_name), optional($.attribute_arg_list)),
    ...
    // Module
    // -------------------------
    generic_param_list_deprecated: ($) =>
      seq("{", commaSep1($._generic_param), "}"), // Deprecated
    module_declaration: ($) =>
      seq(
        ...
        optional(
          choice(
            $.generic_param_list,
            alias($.generic_param_list_deprecated, $.generic_param_list),
          ),
        ),
        optional($.attributes),
        ";",
    ...
    // Import
    // -------------------------
    import_path: ($) => seq($.path_ident, optional($.attributes)),

    import_declaration: ($) =>
      seq(optional($.doc_comment), "import", commaSep1($.import_path), ";"),
      ...
      seq(
        ...
        choice(
          ...
          seq(
            ...
            optional($.attributes),
            choice(seq("=", "module", $.path_ident), $._assign_right_expr),
          ),
          // Constant
          seq(
    ...
    _interface_impl: ($) => choice($.path_type_ident, $.generic_type_ident),
    ...
    bitstruct_member_declaration: ($) =>
      seq(
        ...
        $.ident,
        optional(seq(":", $._expr, optional(seq("..", $._expr)))),
        optional($.attributes),
        ";",
      ),
    bitstruct_body: ($) =>
      seq("{", repeat($.bitstruct_member_declaration), "}"),
    bitstruct_declaration: ($) =>
      seq(
        optional($.doc_comment),
        "bitstruct",
    ...
    // -------------------------
    enum_arg: ($) =>
      choice(seq("=", $._expr), seq("{", commaSepTrailing($._expr), "}")),
    enum_constant: ($) =>
      seq(
        optional($.doc_comment),
        field("name", $.const_ident),
    ...
    enum_param: ($) =>
      seq(optional("inline"), field("type", $.type), field("name", $.ident)),
    ...
    enum_body: ($) => seq("{", commaSepTrailing($.enum_constant), "}"),
    enum_declaration: ($) =>
      seq(
        optional($.doc_comment),
        choice("enum", "constdef", seq("const", "enum")),
        field("name", $.type_ident),
        optional($.interface_impl_list),
        optional($.enum_spec),
        optional($.generic_param_list),
    ...
    interface_func_declaration: ($) =>
      seq(optional($.doc_comment), $.func_declaration),
    interface_body: ($) => seq("{", repeat($.interface_func_declaration), "}"),
    interface_declaration: ($) =>
      seq(
        optional($.doc_comment),
        "interface",
        field("name", $.type_ident),
        optional($.generic_param_list),
        optional(seq(":", commaSep1($.type_ident))),
    ...
    trailing_block_param: ($) => seq($.at_ident, optional($.func_param_list)),
    ...
    macro_param_list: ($) =>
      seq(
        ...
        optional(
          choice(
            $._parameters,
            seq(optional($._parameters), ";", $.trailing_block_param),
          ),
        ),
        ")",
      ),
    ...
    macro_func_body: ($) =>
      choice(seq("=>", $.call_expr), seq($.implies_body, ";"), $.compound_stmt),
    ...
    defer_stmt: ($) =>
      seq(
        "defer",
        optional(choice("try", "catch", $.defer_catch_ident)),
        $._statement,
      ),
    ...
    _decl_ident_or_identifier_list: ($) =>
      prec(1, choice(field("name", $._decl_ident), $.identifier_list)),
    ...
    declaration: ($) =>
      seq(optional($._decl_storage), field("type", $.type), $._decl_after_type),
    ...
    _declaration: ($) => choice($.declaration, $.const_declaration),

    global_declaration: ($) =>
      seq(
        optional($.doc_comment),
        optional("extern"),
        choice(seq($._declaration, ";"), $.func_declaration),
      ),
    ...
    case_stmt: ($) =>
      seq(
        "case",
        field("value", choice($._expr, $.case_range)),
        ":",
        repeat($._statement),
      ),
    ...
    // Default Statement
    // -------------------------
    default_stmt: ($) => seq("default", ":", repeat($._statement)),
    ...
    // -------------------------
    switch_body: ($) =>
      seq("{", repeat(choice($.case_stmt, $.default_stmt)), "}"),
    switch_stmt: ($) =>
      seq(
        "switch",
    ...
    try_unwrap: ($) =>
      prec(
        PREC.LOGICAL_AND + 1,
        seq("try", optional(seq(optional($.type), $.ident, "=")), $._expr),
      ),
    ...
    _try_unwrap_chain: ($) =>
      seq(
        $.try_unwrap,
        repeat(
          prec(PREC.LOGICAL_AND + 1, seq("&&", choice($.try_unwrap, $._expr))),
        ),
      ),
    ...
    else_part: ($) =>
      seq("else", field("body", choice($.if_stmt, $._statement))),
    ...
    for_cond: ($) => seq("(", $._for_cond_inner, ")"),

    for_stmt: ($) =>
      seq("for", optional($.label), $.for_cond, field("body", $._statement)),

    // Foreach Statement
    // -------------------------
    foreach_var: ($) => choice(seq(optional($.type), optional("&"), $.ident)),
    foreach_cond: ($) =>
      seq(
        "(",
    ...
    do_stmt: ($) =>
      seq(
        ...
        optional($.label),
        field("body", $._statement),
        seq(optional(seq("while", field("condition", $.paren_expr))), ";"),
      ),

    // ASM Statement
    // -------------------------
    asm_instr: ($) => seq(choice("int", $.ident), optional(seq(".", $.ident))),
    ...
    asm_stmt: ($) => seq($.asm_instr, commaSep($.asm_expr), ";"),
    asm_block_stmt: ($) =>
      choice(
    ...
    ct_assert_stmt: ($) =>
      choice(
        seq("$assert", $._expr, optional(seq(":", commaSep1($._expr))), ";"),
        seq("$error", $._expr, repeat(seq(",", $._expr)), ";"),
      ),
    ...
    // -------------------------
    ct_exec_stmt: ($) =>
      seq("$exec", "(", commaSep($._expr), ")", optional($.attributes), ";"),
    ...
    ct_case_stmt: ($) =>
      seq(
        choice(seq("$case", $.ct_case_cond, ":"), seq("$default", ":")),
        optional($.ct_stmt_body),
      ),
    ...
    ct_switch_stmt: ($) =>
      seq($._ct_switch, repeat($.ct_case_stmt), "$endswitch"),
    ...
    // -------------------------
    ident_expr: ($) =>
      choice(seq(optional($._module_path), $._expr_ident), $._local_expr_ident),

    _type_expr: ($) => choice($.type, alias($.type_paren_expr, $.paren_expr)),
    type_paren_expr: ($) => prec(2, seq("(", $._type_expr, ")")),

    _ct_call: ($) =>
      choice("$alignof", "$extnameof", "$nameof", "$offsetof", "$qnameof"),

    // Precedence over _expr
    flat_path: ($) => prec(1, seq($._base_expr, optional($.param_path))),

    string_expr: ($) =>
      prec.right(repeat1(choice($.string_literal, $.raw_string_literal))),
    bytes_expr: ($) => repeat1($.bytes_literal),
    paren_expr: ($) => seq("(", $._expr, ")"),

    // Precedence over cast
    typed_initializer_list: ($) =>
      prec(3, seq("(", $._type_expr, ")", $.initializer_list)),
    ...
    _base_expr: ($) =>
      prec(
        ...
        choice(
          ...
          seq(
            choice("$eval", "$is_const", "$sizeof", "$stringify", "$kindof"),
            $.paren_expr,
          ),
    ...
    initializer_list: ($) =>
      seq("{", commaSepTrailing($.initializer_element), "}"),
    ...
    // -------------------------
    lambda_expr: ($) =>
      prec.right(PREC.TERNARY, seq($.lambda_declaration, $.implies_body)),
    ...
    unary_expr: ($) =>
      prec(
        PREC.UNARY,
        seq(field("operator", $._unary_op), field("argument", $._expr)),
      ),
    ...
    _call_args: ($) =>
      choice(
        ...
        seq(commaSepTrailing($.call_arg), seq(";", optional($._parameters))),
      ),
    ...
    call_inline_attributes: ($) =>
      prec.right(
        repeat1(alias(choice("@pure", "@inline", "@noinline"), $.at_ident)),
      ),
    call_expr: ($) =>
      prec.right(
        PREC.TRAILING,
    ...
    update_expr: ($) =>
      prec.right(
        PREC.TRAILING,
        seq(field("argument", $._expr), field("operator", choice("--", "++"))),
      ),
    ...
    trailing_generic_expr: ($) =>
      prec.right(
        PREC.TRAILING,
        seq(field("argument", $._expr), field("operator", $.generic_arg_list)),
      ),

    // Range Expression
    // -------------------------
    _range_loc: ($) => seq(optional("^"), $._expr),
    range_expr: ($) =>
      seq(
    ...
    subscript_expr: ($) =>
      prec.right(
        ...
        seq(
          field("argument", $._expr),
          "[",
          choice(field("index", $._range_loc), field("range", $.range_expr)),
          "]",
        ),
      ),
    ...
    field_expr: ($) =>
      seq(
        prec(PREC.FIELD, seq(field("argument", $._expr), ".")),
        $._access_ident_expr,
      ),
    ...
    type_access_expr: ($) =>
      seq(
        prec(PREC.FIELD, seq(field("argument", $._type_expr), ".")),
        $._access_ident_expr,
      ),
    ...
    generic_type_ident: ($) => seq($._base_type, $.generic_arg_list),
    ...
    type: ($) =>
      prec.right(
        seq(
          choice($._base_type, $.generic_type_ident),
          repeat($.type_suffix),
          optional(choice("~", "?")),
        ),
      ),
    _type_no_generics: ($) =>
      prec.right(
        seq($._base_type, repeat($.type_suffix), optional(choice("~", "?"))),
      ),
  },
});
